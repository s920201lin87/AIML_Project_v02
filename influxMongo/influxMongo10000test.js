...
(async () => {
  ...
  try {
    ...
    while (await cursor.hasNext()) {
      // 如果已插入超過 10,000 筆，停止處理
      if (totalInserted >= 10000) {
        console.log("Reached the limit of 10,000 records. Stopping data migration.");
        break;
      }
      ...
      // 為時間戳增加隨機微秒偏移，確保唯一性
      const randomOffset = Math.floor(Math.random(100000) * 1000000000); // 0 到 1,000,000 微秒
    ...
    // 寫入剩餘數據
    if (batch.length > 0 && totalInserted < 10000) {
      const remainingToInsert = Math.min(batch.length, 10000 - totalInserted);
      await writeBatchWithRetry(batch.slice(0, remainingToInsert), failedBatches);
      totalInserted += remainingToInsert;
    }
